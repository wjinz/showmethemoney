export function Ring({pct=0,size=104,stroke=6,children=undefined,color=undefined,trackColor=undefined}){
  const r=(size-stroke)/2, c=2*Math.PI*r, p=Math.min(pct,100)/100*c;
  const ringColor=color??(pct>100?"var(--red)":pct>85?"#d4b84a":"var(--green)");
  return(
    <div style={{position:"relative",width:size,height:size,flexShrink:0}}>
      <svg width={size} height={size} style={{transform:"rotate(-90deg)",position:"absolute"}}>
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={trackColor??"var(--track)"} strokeWidth={stroke}/>
...
export function Card({children,style={},className="",onClick=undefined}){
