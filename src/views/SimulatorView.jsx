...
const simTooltip=({active,payload,label})=>{
  ...
  return(
    <div style={{background:"var(--surface-alt)",border:"1px solid var(--border)",borderRadius:10,padding:"10px 14px",fontSize:11}}>
      <div style={{color:"var(--text-muted)",marginBottom:4}}>{label}년</div>
  ...
  if (!data || data.length === 0) {
    return (
      <div style={{padding:"40px 20px", textAlign:"center", color:"var(--text-muted)"}}>
        ...
        <button onClick={reset} style={{display:"block", margin:"20px auto", padding:"10px 20px", borderRadius:10, border:"1px solid var(--border)", background:"var(--surface-alt)", color:"var(--text)"}}>초기화</button>
      </div>
    );
  }
    ...
    <div style={{padding:"0 16px 96px",overflowY:"auto",height:"100%", background:"var(--bg)"}}>
      <div className="u1">
        <div style={{padding:"22px 0 4px",display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
          ...
          <button onClick={reset} style={{fontSize:11,color:"var(--text-muted)",background:"rgba(255,255,255,.04)",border:"1px solid var(--border)",borderRadius:8,padding:"5px 12px",cursor:"pointer",marginBottom:14}}>전체 초기화</button>
        </div>
      </div>

      <Card className="u2" style={{padding:"20px",marginBottom:10}}>
        <div style={{fontSize:11,color:"var(--text-muted)",letterSpacing:".06em",marginBottom:16}}>■ 변수 조정</div>
        <SliderRow label="초기 투자금" value={init} min={1000000} max={200000000} step={1000000}
          onChange={setInit} fillColor="var(--primary)"
        ...
        <div style={{borderTop:"1px solid var(--border)",paddingTop:16,marginTop:4}}>
          ...
          <div style={{
            background:goalYear>0?"var(--success-bg1)":"var(--danger-bg1)",
            border:`1px solid ${goalYear>0?"rgba(77,171,135,.3)":"rgba(217,95,95,.3)"}`,
            borderRadius:10,padding:"10px 14px",fontSize:12,
            color:goalYear>0?"var(--success)":"var(--danger)"
          }}>
      ...
      <div className="u3" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
        {[
          {label:"최종 금액",  val:fmtS(final.복리성장)+"원",c:"var(--primary)", icon:"💰"},
          {label:"복리 추가수익",val:fmtS(final.복리성장-final.단순저축)+"원",c:"var(--success)",icon:"📈"},
      ...
      <Card className="u4" style={{padding:"18px"}}>
        <div style={{fontSize:11,color:"var(--text-muted)",letterSpacing:".06em",marginBottom:14}}>■ 성장 곡선</div>
        <div style={{height:210}}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{top:5,right:8,left:0,bottom:5}}>
              <XAxis dataKey="year" tick={{fill:"var(--text-faint)",fontSize:10}} tickLine={false} axisLine={false} tickFormatter={v=>v+"년"}/>
              <YAxis tick={{fill:"var(--text-faint)",fontSize:10}} tickLine={false} axisLine={false} tickFormatter={fmtS} width={40}/>
